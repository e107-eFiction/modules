...
if($confirm == "yes") {
	...
	dbquery("DELETE FROM `".TABLEPREFIX."fanfiction_codeblocks` WHERE code_module = 'storybanner'");	
	...
	$modquery = dbquery("SELECT * FROM ".TABLEPREFIX. "fanfiction_codeblocks WHERE code_module = 'storybanner'");
	if(dbnumrows($modquery)) {
		$output = write_message(_CONFIRMUNINSTALL."<a href='uninstall.php?confirm=yes'>"._YES."</a> "._OR." <a href='uninstall.php?confirm=no'>"._NO."</a>");
		$output .= "<div class=\"text-center alert alert-warning\">This will not remove banner field from fanfiction story or series tables. Do it manually if it is needed.</div>";
	}
